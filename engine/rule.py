...
#                                 lung_segmentation_nested = outputs[index]
                                ...
                                if s < 5:
                                    save_path=f'/vinbrain/tientran/feedbackloop/FB_pipeline_v2/data_test/lungrule/{local_data_image_url[index]}.jpeg'
                                    imageio.imwrite(save_path,self.segments.visualize(local_image_origin[index], lung_segmentation_nested))      
                    ...
                    # import os
                    # if os.path.isfile(self.path_save_result):
                    #     df_save = pd.read_csv(self.path_save_result)
                    #     df_result = pd.DataFrame(self.dictionary_result)
                    #     df = pd.concat([df_save, df_result])
                    #     df.to_csv(self.path_save_result, index = False)
                    # else:
                    #     df_result = pd.DataFrame(self.dictionary_result)
                    #     df_result.to_csv(self.path_save_result, index = False)
                    pd.DataFrame(dict([(k,pd.Series(v)) for k,v in self.dictionary_result.items()])).to_csv(self.path_save_result, index = False)
#         return self.dictionary_result

...
class RuleBaseEngine():
    def __init__(self, config_path=None):
        ...
        self.df = pd.read_csv(config["path_dataset"])[:2000]
...
#                         try:
                        if label_index[index] == 1 or label_index[index] == 3 :
                            ...
                            save_path = f'/vinbrain/tientran/feedbackloop/FB_pipeline_v2/data_test/image_with_lungmask/{local_data_image_id[index]}'
                            imageio.imwrite(save_path + '.jpeg',self.segments.visualize(local_image_origin[index], lung_segmentation_nested))
                                ##

...
def default_collate(batch):
    ...
    tenant_code = [item[6] for item in batch]
    records = [item[7] for item in batch]
...
class Dataset(data.Dataset):
    def __init__(self, df_info, disease_names, img_size, crop_size):
...
class RuleBaseEngine():
    def __init__(self, config_path=None):
        ...
        self.df = pd.read_csv(config["path_dataset"])[30000:]
        self.dictionary_process = {
             "airspace_opacity": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
                'thresh_rm_small': (6000, 80, 120)
            },
            "lung_opacity": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
                'thresh_rm_small': (8000, 80, 120)
            },
            "lung_lesion": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
            ...
            "atelectasis": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
                'thresh_rm_small': (6000, 150, 100)
            },
            "consolidation": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
                'thresh_rm_small': (2000, 200, 200)
            },
            "pneumonia": {
                "rule_function": inside_lung_process,
                "model_other_segmentation": None,
        ...
        self.dictionary_result = {k: [] for k in ["image_id", "image_url", 'tenant_code'] + self.disease_names}
        self.params = {
            'batch_size': 64,
            'shuffle': False,
            'num_workers': 6
        ...
        with torch.no_grad():
            with tqdm(data_generator, unit="batch") as tepoch:
                for local_image_origin, local_data_image_id, \
                    ...
                    local_image_chest_cls, local_image_lung_segment, \
                    local_tenant_code, local_records in tepoch:
                    ...
                    for index, image_origin in enumerate(local_image_origin):
                        ...
                        self.dictionary_result["tenant_code"].append(local_tenant_code[index])
                        ...
                        if label_index[index] == 1:
                            ...
                            if height < 350 or width < 350:
                                for disease in self.disease_names:
                                    self.dictionary_result[disease].append(-5)
                                continue
                            # end
                            try:
                            ...
                            for disease_name in self.disease_names:
                                if lung_segmentation_nested is not None:
                                    ...
                                    if not pd.isnull(mask_doctor) and pd.isna(mask_doctor) == False and mask_doctor != '[{"polygon":null}]' and mask_doctor != '[{"polygon":[]}]' and mask_doctor != '[]':
                                        mask_doctor = np.array([mask_doctor])
                                        try:
                                            dr_border = mask_image(mask_doctor[0], height, width).reshape((height,width))
                                        except:
                                            print(local_data_image_id[index])
                                            print(mask_doctor)
                                        ...
                                        try:
                                            status = int(self.dictionary_process[disease_name]["rule_function"](
                                                    ...
                                                    thresh_rm_small = self.dictionary_process[disease_name]["thresh_rm_small"]
                                                )
                                            )
                                            self.dictionary_result[disease_name].append(status)
                                        except:
                                            self.dictionary_result[disease_name].append(-10)
                                    else:
                                        if local_records[index][disease_name] == 1:
                        ...
                        elif label_index[index] == 3:
                            for disease_name in self.disease_names:
                                self.dictionary_result[disease_name].append(-7)
                        else:
                            for disease_name in self.disease_names:
                                self.dictionary_result[disease_name].append(-2)
